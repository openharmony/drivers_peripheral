...
public:
    ...
    bool SetUint64Value(AttributeKey key, uint64_t attrValue);
    bool SetUint8ArrayValue(AttributeKey key, const std::vector<uint8_t> &attrValue);

    bool GetUint64Value(AttributeKey key, uint64_t &attrValue) const;
    bool GetUint8ArrayValue(AttributeKey key, std::vector<uint8_t> &attrValue) const;
...
private:
    ...
    static bool EncodeUint64Value(uint64_t src, std::vector<uint8_t> &dstValue);
    static bool EncodeUint32Value(uint32_t src, std::vector<uint8_t> &dst);
    static bool EncodeUint8ArrayValue(const std::vector<uint8_t> &src, std::vector<uint8_t> &dstValue);

    static bool DecodeUint64Value(const std::vector<uint8_t> &src, uint64_t &dstValue);
    static bool DecodeUint8ArrayValue(const std::vector<uint8_t> &src, std::vector<uint8_t> &dstValue);
...
bool Attributes::Impl::SetUint64Value(AttributeKey key, uint64_t attrValue)
{
...
bool Attributes::Impl::SetUint8ArrayValue(AttributeKey key, const std::vector<uint8_t> &attrValue)
{
    std::vector<uint8_t> dest;
    ...
    auto ret = map_.try_emplace(key, attrValue);
    return ret.second;
}

bool Attributes::Impl::GetUint64Value(AttributeKey key, uint64_t &attrValue) const
{
...
bool Attributes::Impl::GetUint8ArrayValue(AttributeKey key, std::vector<uint8_t> &attrValue) const
{
    auto iter = map_.find(key);
    if (iter == map_.end()) {
        return false;
...
{
    uint32_t size = 0;
    for (const auto &[key, attrValue] : map_) {
...
bool Attributes::Impl::EncodeUint64Value(uint64_t src, std::vector<uint8_t> &dstValue)
{
...
bool Attributes::Impl::EncodeUint8ArrayValue(const std::vector<uint8_t> &src, std::vector<uint8_t> &dstValue)
{
...
bool Attributes::Impl::DecodeUint64Value(const std::vector<uint8_t> &src, uint64_t &dstValue)
{
    if (src.size() * sizeof(uint8_t) != sizeof(uint64_t)) {
        return false;
    }

    if (memcpy_s(&dstValue, sizeof(dstValue), src.data(), src.size() * sizeof(uint8_t)) != EOK) {
        return false;
    }
    return true;
}

bool Attributes::Impl::DecodeUint8ArrayValue(const std::vector<uint8_t> &src, std::vector<uint8_t> &dstValue)
{
    std::vector<uint8_t> out(src);
    dstValue.swap(out);
    return true;
...
bool Attributes::SetUint64Value(AttributeKey key, uint64_t attrValue)
{
    if (!impl_) {
        return false;
    }
    return impl_->SetUint64Value(key, attrValue);
}

bool Attributes::SetUint8ArrayValue(AttributeKey key, const std::vector<uint8_t> &attrValue)
{
    if (!impl_) {
        return false;
    }
    return impl_->SetUint8ArrayValue(key, attrValue);
}

bool Attributes::GetUint64Value(AttributeKey key, uint64_t &attrValue) const
{
    if (!impl_) {
        return false;
    }
    return impl_->GetUint64Value(key, attrValue);
}

bool Attributes::GetUint8ArrayValue(AttributeKey key, std::vector<uint8_t> &attrValue) const
{
    if (!impl_) {
        return false;
    }
    return impl_->GetUint8ArrayValue(key, attrValue);
}

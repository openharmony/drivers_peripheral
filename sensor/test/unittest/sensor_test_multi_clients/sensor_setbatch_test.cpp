...
namespace {
    ...
    HWTEST_F(SensorSetBatchTest, SensorSetBatchTest1, TestSize.Level1)
    {
        ...
        int64_t samplingInterval = 100000000;
        ...
        if (hdfSensorInformation.minDelay > samplingInterval || hdfSensorInformation.maxDelay < samplingInterval) {
            GTEST_SKIP() << "samplingInterval not within the frequency range supported by the device" << std::endl;
        }
        ret = g_sensorInterface->Register(0, g_traditionalCallback);
        EXPECT_EQ(ret, HDF_SUCCESS);
        ret = g_sensorInterface->SetBatch(deviceSensorInfo, samplingInterval, 0);

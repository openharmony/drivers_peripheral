...
namespace {
     ...
     * @tc.desc: Advanced multi-client test with different sampling intervals.
     * @tc.type: FUNC
     * @tc.require: #I4L3LF
     */
    HWTEST_F(SensorSetBatchTest, SensorSetBatchTest1, TestSize.Level1)
    {
        ...
        if (info.size() == 0) {
            GTEST_SKIP() << "Sensor not Exist" << std::endl;
        } else {
        ...
        printf("Client running with sampling interval: %lld ns\n", g_samplingInterval);
        ...
        printf("Duration: %d ms, SamplingInterval: %lld ns\n", g_duration, g_samplingInterval);
        ...
        printf("Actual sensor data count: %d\n", SensorCallbackImpl::sensorDataCount);
        
        if (SensorCallbackImpl::sensorDataCount > expectedMinCount &&
            SensorCallbackImpl::sensorDataCount < expectedMaxCount) {
            printf("\033[32m[PASS] %dms get sensor data count is %d (expected range: %d-%d)\033[0m\r\n",
                   g_duration, SensorCallbackImpl::sensorDataCount, expectedMinCount, expectedMaxCount);
        } else {
            printf("\033[31m[FAIL] %dms get sensor data count is %d (expected range: %d-%d)\033[0m\r\n",
                   g_duration, SensorCallbackImpl::sensorDataCount, expectedMinCount, expectedMaxCount);
        }
    }
}

...
namespace {
    ...
    constexpr int64_t CLIENT3_SAMPLING_INTERVAL = 10000000;
    constexpr int32_t TOTAL_SECONDS = 6;
    constexpr int32_t EXPECTED_MIN_COUNT_PER_SECOND = 90;

    class SensorFourClientsClient3Test : public testing::Test {
    public:
    ...
    void SensorFourClientsClient3Test::SetUpTestCase()
    {
    ...
    void SensorFourClientsClient3Test::TearDownTestCase()
    {
    }

    void SensorFourClientsClient3Test::SetUp()
    {
    }

    void SensorFourClientsClient3Test::TearDown()
    {
    }

    /**
     * @tc.name: SensorFourClientsClient3Test1
     * @tc.desc: Client3 subscribes sensor at 10ms via SetBatch, counts data for 6 seconds.
     * @tc.type: FUNC
     * @tc.require: #I4L3LF
     */
    HWTEST_F(SensorFourClientsClient3Test, SensorFourClientsClient3Test1, TestSize.Level1)
    {
        setvbuf(stdout, nullptr, _IONBF, 0);
        ASSERT_NE(nullptr, g_sensorInterface);

        printf("[Client3] calling Register\n");
        fflush(stdout);
        int32_t ret = g_sensorInterface->Register(0, g_traditionalCallback);
        printf("[Client3] Register ret=%d\n", ret);
        EXPECT_EQ(ret, HDF_SUCCESS);

        printf("[Client3] calling SetBatch interval=%sns\n", std::to_string(CLIENT3_SAMPLING_INTERVAL).c_str());
        ret = g_sensorInterface->SetBatch(g_deviceSensorInfo, CLIENT3_SAMPLING_INTERVAL, 0);
        printf("[Client3] SetBatch ret=%d\n", ret);
        EXPECT_EQ(ret, HDF_SUCCESS);

        printf("[Client3] calling Enable\n");
        ret = g_sensorInterface->Enable(g_deviceSensorInfo);
        printf("[Client3] Enable ret=%d\n", ret);
        EXPECT_EQ(ret, HDF_SUCCESS);

        printf("[Client3] sensor enabled, start counting for %d seconds (expected min %d/s)\n",
        ...
        for (int32_t i = 0; i < TOTAL_SECONDS; i++) {
            ...
            if (countPerSecond >= EXPECTED_MIN_COUNT_PER_SECOND) {
                printf("\033[32m[Client3][%ds] OK, 1s data count=%d, total=%d\033[0m\n",
                    i + 1, countPerSecond, SensorCallbackImpl::sensorDataCount);
            } else {
                printf("\033[31m[Client3][%ds] [ERROR] 1s data count=%d (expected>=%d), total=%d\033[0m\n",
                    i + 1, countPerSecond, EXPECTED_MIN_COUNT_PER_SECOND, SensorCallbackImpl::sensorDataCount);
            }
        }

        printf("[Client3] calling Disable\n");
        ret = g_sensorInterface->Disable(g_deviceSensorInfo);
        printf("[Client3] Disable ret=%d\n", ret);
        EXPECT_EQ(ret, HDF_SUCCESS);

        printf("[Client3] calling Unregister\n");
        ret = g_sensorInterface->Unregister(0, g_traditionalCallback);
        printf("[Client3] Unregister ret=%d\n", ret);
        EXPECT_EQ(ret, HDF_SUCCESS);

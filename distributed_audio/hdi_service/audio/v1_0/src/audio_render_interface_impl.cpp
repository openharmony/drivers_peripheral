...
int32_t AudioRenderInterfaceImpl::RenderFrame(const std::vector<int8_t> &frame, uint64_t &replyBytes)
{
    ...
    int32_t ret = audioExtCallback_->WriteStreamData(adapterName_, devDesc_.pins, renderId_, data);
    if (ret != HDF_SUCCESS) {
...
int32_t AudioRenderInterfaceImpl::Start()
{
    ...
    if (firstOpenFlag_) {
        ...
        int32_t ret = audioExtCallback_->NotifyEvent(adapterName_, devDesc_.pins, renderId_, event);
        if (ret != HDF_SUCCESS) {

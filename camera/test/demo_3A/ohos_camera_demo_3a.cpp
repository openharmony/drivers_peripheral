...
RetCode OhosCameraDemo::InitCameraDevice()
{
    ...
    const std::string cameraId = cameraIds_.back();
...
#endif
    rc = demoCameraHost_->OpenCamera(cameraIds_.back(), callback, demoCameraDevice_);
...
void OhosCameraDemo::SetAwbMode()
{
    int32_t awbMode;
    ...
    if (awbStatus_) {
        awbMode = OHOS_CAMERA_AWB_MODE_AUTO;
        printf("set auto awb mode on\n");
    } else {
        awbMode = OHOS_CAMERA_AWB_MODE_OFF;
        printf("set auto awb mode off\n");
    }

    awbStatus_ = !awbStatus_;
    metaData->addEntry(OHOS_CONTROL_AWB_MODE, &awbMode, 1);

    const int32_t deviceStreamId = 0;
    metaData->addEntry(OHOS_CAMERA_STREAM_ID, &deviceStreamId, 1);
...
void OhosCameraDemo::SetAfAuto()
{
    int32_t afMode;

    CAMERA_LOGD("demo test: SetAfAuto enter");
    ...
    if (afStatus_) {
        afMode = OHOS_CAMERA_FOCUS_MODE_CONTINUOUS_AUTO;
        printf("set auto focus mode on\n");
    } else {
        afMode = OHOS_CAMERA_FOCUS_MODE_MANUAL;
        printf("set auto focus mode off\n");
    }
    afStatus_ = !afStatus_;
    metaData->addEntry(OHOS_CONTROL_FOCUS_MODE, &afMode, 1);
    ...
    CAMERA_LOGD("demo test: SetAfAuto exit");
}
...
void OhosCameraDemo::SetAeAuto()
{
    int32_t expo;

    CAMERA_LOGD("demo test: SetAeAuto enter");
    ...
    if (aeAutoStatus_) {
        expo = OHOS_CAMERA_EXPOSURE_MODE_CONTINUOUS_AUTO;
        printf("set auto exposure mode on\n");
    } else {
        expo = OHOS_CAMERA_EXPOSURE_MODE_MANUAL;
        printf("set auto exposure mode off\n");
    }
    aeAutoStatus_ = !aeAutoStatus_;
    metaData->addEntry(OHOS_CONTROL_EXPOSURE_MODE, &expo, 1);
    const int32_t deviceStreamId = 0;
    ...
    CAMERA_LOGD("demo test: SetAeAuto exit");
}

void OhosCameraDemo::SetAELock()
{
    int32_t aeLock;

    CAMERA_LOGD("demo test: SetAELock enter");
    ...
    if (aeLockStatus_) {
        aeLock = OHOS_CAMERA_AE_LOCK_ON;
        printf("set auto exposure lock on\n");
    } else {
        aeLock = OHOS_CAMERA_AE_LOCK_OFF;
        printf("set auto exposure lock off\n");
    }
    aeLockStatus_ = !aeLockStatus_;

    metaData->addEntry(OHOS_CONTROL_AE_LOCK, &aeLock, 1);
    ...
    CAMERA_LOGD("demo test: SetAELock exit");
}

void OhosCameraDemo::SetAWBLock()
{
    int32_t awbLock;

    CAMERA_LOGD("demo test: SetAWBLock enter");
    ...
    if (awbLockStatus_) {
        ...
        awbLock = OHOS_CAMERA_AWB_LOCK_OFF;
        printf("set auto exposure lock off\n");
    }
    awbLockStatus_ = !awbLockStatus_;

    metaData->addEntry(OHOS_CONTROL_AWB_LOCK, &awbLock, 1);
    ...
    CAMERA_LOGD("demo test: SetAWBLock exit");
}

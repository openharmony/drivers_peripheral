...
void Test::StartStream(std::vector<StreamIntent> intents)
{
    ...
    for (auto& intent : intents) {
        if (intent == StreamIntent::PREVIEW) {
            ...
            streamInfoAnalyze->v1_0.bufferQueue_ = consumer_analyze->CreateProducerSeq([this](void* addr, uint32_t size) {
            ...
            streamInfoCapture->v1_0.bufferQueue_ = consumer_capture->CreateProducerSeq([this](void* addr, uint32_t size) {
                DumpImageFile(streamIdPreview, "yuv", addr, size);
            });

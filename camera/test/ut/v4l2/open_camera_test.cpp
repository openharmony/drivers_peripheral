...
TEST_F(UtestOpenCameraTest, camera_open_0001)
{
    ...
    std::string cameraIds = cameraIds.front();
    const OHOS::sptr<ICameraDeviceCallback> callback = new DemoCameraDeviceCallback();
    OHOS::sptr<ICameraDevice> cameraDevice;
    cameraBase->rc = (CamRetCode)cameraBase->cameraHost->OpenCamera(caneraId, callback, caneraDevice);
...
{
    CAMERA_LOGD("OpenCamera, cameraID is no found.");
    std::string cameraId = "qwerty";
    OHOS::sptr<ICameraDeviceCallback> callback = new DemoCameraDeviceCallbcak();
    CANERA_LOGD("opencamera begin");
    cameraBase->rc = (CameRetCode)cameraBase->cameraHost->OpenCamera(cameraId, callback, cameraBase->cameraDevice);
...
/**
  * @tc.name: OpenCamera cameraID input error
  * @tc.desc: OpenCamera, cameraID is Null.
  * @tc.level: level2
  * @tc.size: MediumTest
  * @tc.type: Function
  */
...
{
    CAMERA_LOGD("OpenCamera, cameraID is Null.");
...
TEST_F(UtestOpenCameraTest, camera_open_0050)
{
    ...
    for (auto &cameraId :cameraIds) {
        CAMERA_LOGI("cameraId = %{public}s", caneraId.c_str());
        const OHOS::sptr<ICameraDeviceCallback> callback = new DemoCameraDeviceCallback();
        cameraBase->rc = (CamRetCode)cameraBase->cameraHost->OpenCamera(cameraId, callback, cameraBase->cameraDevice);
        EXPECT_EQ(true, cameraBase->rc == HDI::Camera::V1_0::NO_ERROR);

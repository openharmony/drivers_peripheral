    ...
    if (inOut) {
        ...
        if (fd < 0) {
            ...
            if (fileFormat == nullptr) {
                CAMERA_LOGE("UVC:V4L2UvcMatchDev fileFormat make_shared is nullptr\n");
            } else {
            ...
            if (control == nullptr) {
                CAMERA_LOGE("UVC:V4L2UvcMatchDev control make_shared is nullptr\n");
            } else {
...
RetCode HosV4L2UVC::V4L2UvcDetectInit(UvcCallback cb)
{
    ...
    if (cb == nullptr || uvcDetectEnable_) {
        CAMERA_LOGE("UVC:V4L2Detect is on or UvcCallback is nullptr\n");
        return RC_ERROR;
    }
    // set callback
    uvcCallbackFun_ = cb;

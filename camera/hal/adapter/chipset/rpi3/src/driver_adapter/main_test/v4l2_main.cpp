    ...
    const std::shared_ptr<FrameSpec>& buffer)
{
    ...
    if (buffer->bufferPoolId_ == 0) {
        sprintf_s(path, sizeof(path), "./raspberry%d.jpeg", index);
    }
...
void ProcessImage(const unsigned char* p, unsigned char* fbp)
{
    ...
    for (y = ypos; y < (height + ypos); y++) {
        for (j = 0, x = xpos; j < width; j++, x++) {
            location = (x + g_vInfo.xoffset) * (g_vInfo.bits_per_pixel / 8) +
                ...
                if (iter->type == V4L2_CTRL_TYPE_MENU) {
                    for (auto itr = iter->menu.cbegin(); itr != iter->menu.cend(); itr++) {
                        CAMERA_LOGD("main test: %s index %d id 0x%x value %lld\n",
                            itr->name.c_str(), itr->index, itr->id, itr->value);
                    }
                }
            }
...
void V4L2SetDeviceFormat(DeviceFormat& format, const std::string devname)
{
    ...
    if (devname == "bm2835 mmal") {
        ...
        if (g_isVideoOn) {
            format.fmtdesc.pixelformat = V4L2_PIX_FMT_H264;
            format.fmtdesc.width = captureWidth;
            format.fmtdesc.height = captureHeight;
        }
    }
...
RetCode V4L2InitSensors()
{
    ...
    cameraIDs.push_back("bm2835 mmal");

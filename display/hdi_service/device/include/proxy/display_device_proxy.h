...
#ifndef _HDI_DISPLAY_HOST_PROXY_H_
#define _HDI_DISPLAY_HOST_PROXY_H_
...
namespace Device {
namespace Client {
#define GET_INSTANCE DisplayComposerProxy::GetInstance
using OHOS::MessageOption;
using OHOS::MessageParcel;
using std::unique_ptr;
class DisplayDeviceProxy : public IRemoteProxy<IDisplayDevice> {
public:
    explicit DisplayDeviceProxy(const sptr<IRemoteObject> &impl) : IRemoteProxy<IDisplayDevice>(impl)
    {
    }
    ~DisplayDeviceProxy() = default;
    static OHOS::sptr<IDisplayDevice> GetInstance(void);
...
private:
    static inline OHOS::BrokerDelegator<DisplayDeviceProxy> delegator_;
    void ReleaseInstance(void);
    ComposerServerTmp *GetReturnData(DisplayDeviceCommandId cmdId) const;
    int32_t DisplayComposerExecuteCmd(DisplayDeviceCommandId cmdId, MessageParcel *dataParcel, MessageParcel *repParcel,
        void *data1 = nullptr, void *data2 = nullptr);
    void DeviceReplyDataDistributer(
        DisplayDeviceCommandId cmdId, MessageParcel *pReplay, void *data1 = nullptr, void *data2 = nullptr);
};
} // namespace Client
} // namespace Device
} // namespace Display
} // namespace OHOS
#endif // _HDI_DISPLAY_HOST_PROXY_H_

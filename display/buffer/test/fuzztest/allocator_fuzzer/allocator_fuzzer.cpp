...
void TestCloneDmaBufferHandle(const BufferHandle& handle)
{
    BufferHandle* outHandle = nullptr;
    (void)g_bufferInterface->CloneDmaBufferHandle(handle, outHandle);
...
bool FuzzTest(const uint8_t* rawData, size_t size)
{
    ...
    if (!g_isInit) {
        g_isInit = true;
        g_bufferInterface.reset(OHOS::HDI::Display::Buffer::V1_4::IDisplayBuffer::Get());

...
{
    static const ValueStrMap PIXEL_STR_MAPS[] = {
    ...
    for (uint32_t i = 0; i < sizeof(PIXEL_STR_MAPS) / sizeof(PIXEL_STR_MAPS[0]); i++) {
        if (PIXEL_STR_MAPS[i].value == static_cast<uint32_t>(format)) {
            return PIXEL_STR_MAPS[i].str;
        }
    }
...
{
    static const ValueStrMap FORMAT_STR_MAPS[] = {
    ...
    for (uint32_t i = 0; i < sizeof(FORMAT_STR_MAPS) / sizeof(FORMAT_STR_MAPS[0]); i++) {
        if (FORMAT_STR_MAPS[i].value == format) {
...
static uint32_t ConvertFormatToDrm(PixelFormat fmtIn)
{
    ...
    for (uint32_t i = 0; i < sizeof(CONVERT_TABLE) / sizeof(CONVERT_TABLE[0]); i++) {
        if (CONVERT_TABLE[i].pixFormat == fmtIn) {
            fmtOut = CONVERT_TABLE[i].drmFormat;
        }
    }
...
static int32_t InitGbmDevice(const char *drmFile, GrallocManager *grallocManager)
{
    ...
    if (grallocManager->gbmDevice == nullptr) {
        ...
        drmDropMaster(drmFd);
        struct gbm_device *gbmDevice = HdiGbmCreateDevice(drmFd);
...
{
    DISPLAY_LOGD();
    HdiGbmDeviceDestroy(grallocManager->gbmDevice);
...
int32_t GbmAllocMem(const AllocInfo *info, BufferHandle **buffer)
{
    ...
    DISPLAY_CHK_RETURN((grallocManager == nullptr), HDF_ERR_INVALID_PARAM, DISPLAY_LOGE("gralloc manager failed");
        GRALLOC_UNLOCK());
    struct gbm_bo *bo =
        HdiGbmBoCreate(grallocManager->gbmDevice, info->width, info->height, drmFmt, ConvertUsageToGbm(info->usage));
    DISPLAY_CHK_RETURN((bo == nullptr), HDF_DEV_ERR_NO_MEMORY, DISPLAY_LOGE("gbm create bo failed"); \
        GRALLOC_UNLOCK());

    int fd = HdiGbmBoGetFd(bo);

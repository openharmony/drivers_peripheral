...
std::shared_ptr<IDisplayBufferHwi> g_dispbuf;
int32_t rkInitGfx()
{
    DISPLAY_LOGE("%s\n", querystring(RGA_ALL));
...
int32_t doFlit(ISurface *srcSurface, IRect *srcRect, ISurface *dstSurface, IRect *dstRect, GfxOpt *opt)
{
    ...
    DISPLAY_LOGD("gfx src fd %{public}d, w %{public}d, h %{publuc}d, sw %{public}d sh %{public}d vir %{public}p",
    ...
    DISPLAY_LOGD("gfx dst fd %{public}d, w %{public}d, h %{public}d, sw %{public}d sh %{public}d vir %{public}p",
    ...
    if (isYuv == 1) {
        if (rkBlendType == IM_ALPHA_BLEND_SRC_OVER || rkBlendType == IM_ALPHA_BLEND_SRC) {
            ...
            int32_t ec = g_dispbuf->AllocMem(info, buffer);
            DISPLAY_CHK_RETURN((ec != HDF_SUCCESS), DISPLAY_FAILURE, DISPLAY_LOGE("can not alloc memory"));

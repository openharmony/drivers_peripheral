...
int32_t HdiTestLayer::PreparePresent()
{
    int ret;
    DISPLAY_TEST_LOGD();
    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerPosition(mDisplayID, mId, mDisplayRect);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE, DISPLAY_TEST_LOGE("set display rect failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerCrop(mDisplayID, mId, mCropRect);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE, DISPLAY_TEST_LOGE("set display crop failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerZorder(mDisplayID, mId, mZorder);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE, DISPLAY_TEST_LOGE("set display zorder failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerCompositionType(mDisplayID, mId, mCompType);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE,
        DISPLAY_TEST_LOGE("set display composition type failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetTransformMode(mDisplayID, mId, mTransform);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE, DISPLAY_TEST_LOGE("set transform mode failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerAlpha(mDisplayID, mId, mAlpha);
    ...
    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerBuffer(mDisplayID, mId, *handle, -1);
    DISPLAY_TEST_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE, DISPLAY_TEST_LOGE("set buffer handle failed"));

    ret = HdiTestDevice::GetInstance().GetDeviceInterface()->SetLayerBlendType(mDisplayID, mId, mBlendType);

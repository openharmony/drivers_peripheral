...
#include "prepare_display_layers_fuzzer.h"
...
bool FuzzTest(const uint8_t *data, size_t size)
{
    ...
    info.width = width;
    info.height = height;
    info.type = LayerType::LAYER_TYPE_GRAPHIC;
    info.pixFormat = PixelFormat::PIXEL_FMT_RGBA_8888;

    auto ret = composerDevice->CreateLayer(*(uint32_t *)data, info, layerId);
    ret = composerDevice->PrepareDisplayLayers(*(uint32_t *)data, needFlushFb);
    if (!ret) {
        result = true;
    }
    return result;

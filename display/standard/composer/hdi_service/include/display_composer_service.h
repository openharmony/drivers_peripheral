...
public:
    ...
    int32_t GetDisplayCompChange(uint32_t devId, std::vector<uint32_t>& layers,
         std::vector<int32_t>& type) override;
    ...
    int32_t GetDisplayReleaseFence(uint32_t devId, std::vector<uint32_t>& layers,
         std::vector<sptr<HdifdParcelable>>& fences) override;
    ...
    int32_t SetVirtualDisplayBuffer(uint32_t devId, const sptr<BufferHandleParcelable>& buffer,
         const sptr<HdifdParcelable>& fence) override;

    ...
    std::vector<int32_t>& types)
{
    ...
    if (ec == HDF_SUCCESS && num != 0) {
        if (num > MAX_DISPLAY_LAYERS || num > MAX_DISPLAY_TYPES) {
            DISPLAY_LOGE("layer/type count exceeds maxnum limit: %u > %u or %u", num,
                MAX_DISPLAY_LAYERS, MAX_DISPLAY_TYPES);
            return HDF_ERR_INVALID_PARAM;
        }
        layers.resize(num);
        types.resize(num);
    ...
    std::vector<int32_t>& fences)
{
    ...
    if (ec == HDF_SUCCESS && num != 0) {
        if (num > MAX_DISPLAY_LAYERS || num > MAX_DISPLAY_FENCES) {
            DISPLAY_LOGE("layer/fence count exceeds maxnum limit: %u > %u or %u", num,
                MAX_DISPLAY_LAYERS, MAX_DISPLAY_FENCES);
            return HDF_ERR_INVALID_PARAM;
        }
        layers.resize(num);
        fences.resize(num);

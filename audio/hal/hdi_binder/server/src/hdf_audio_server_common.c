...
static bool GetAudioRouteNodeMem(struct HdfSBuf *data, struct AudioRouteNode* source, uint32_t *num)
{
    ...
    if (*num > 0) {
        source = (struct AudioRouteNode*)OsalMemCalloc(sizeof(struct AudioRouteNode) * (*num));
        if (source == NULL) {
            return false;
        }
        for (uint32_t i = 0; i < *num; i++) {
            if (!AudioRouteNodeBlockUnmarshalling(data, &source[i])) {
...
static bool AudioRouteBlockUnmarshalling(struct HdfSBuf *data, struct AudioRoute *dataBlock)
{
    ...
    if (!GetAudioRouteNodeMem(data, sourcesCp, &sourcesNum)) {
        return false;
    }
    dataBlock->sources = sourcesCp;
    dataBlock->sourcesNum = sourcesNum;

    if (!GetAudioRouteNodeMem(data, sinksCp, &sinksNum)) {
        return false;
    }

...
static struct AudioCapturePrivVdi g_audioCapturePrivVdi = {0};
...
{
    return &g_rwVdiCaptureLock[indexId];
}
...
int32_t AudioCaptureStartVdi(struct IAudioCapture *capture)
{
    ...
    struct AudioCaptureInfo *captureInfo = (struct AudioCaptureInfo *)(capture);
    uint32_t captureId = captureInfo->captureId;
    AUDIO_FUNC_LOGI("hdi start enter, captureId=%{public}u", captureId);
    if (captureId >= AUDIO_VDI_STREAM_NUM_MAX) {
        AUDIO_FUNC_LOGE("invalid param");
        return HDF_ERR_INVALID_PARAM;
    }
...
int32_t AudioCaptureStopVdi(struct IAudioCapture *capture)
{
    ...
    struct AudioCaptureInfo *captureInfo = (struct AudioCaptureInfo *)(capture);
    uint32_t captureId = captureInfo->captureId;
    AUDIO_FUNC_LOGI("hdi stop enter, captureId=%{public}u", captureId);
    if (captureId >= AUDIO_VDI_STREAM_NUM_MAX) {
        AUDIO_FUNC_LOGE("invalid param");
        return HDF_ERR_INVALID_PARAM;
    }
...
static int32_t AudioCaptureRegCallbackVdi(struct IAudioCapture *capture, struct IAudioCaptureCallback *audioCallback)
{
    ...
    struct AudioCaptureInfo *captureInfo = (struct AudioCaptureInfo *)(capture);
    uint32_t captureId = captureInfo->captureId;
    AUDIO_FUNC_LOGI("AudioCaptureRegCallbackVdi hdi RegCallback enter, captureId=%{public}u", captureId);
    if (captureId >= AUDIO_VDI_STREAM_NUM_MAX) {
        AUDIO_FUNC_LOGE("invalid param");
        return HDF_ERR_INVALID_PARAM;
    }
    ...
    struct IAudioCaptureVdi *vdiCapture = captureInfo->vdiCapture;
    if (vdiCapture == NULL || vdiCapture->RegCallback == NULL) {
        AUDIO_FUNC_LOGE("invalid param");
        pthread_rwlock_unlock(&g_rwVdiCaptureLock[captureId]);
        return HDF_ERR_INVALID_PARAM;
    }
    ...
    struct IAudioCaptureVdi *vdiCapture, const struct AudioDeviceDescriptor *desc)
{
    ...
    *captureId = GetAvailableCaptureId(priv);
    ...
    pthread_rwlock_unlock(&g_rwVdiCaptureLock[*captureId]);
    AUDIO_FUNC_LOGD("audio create capture success, captureId=%{public}u", *captureId);
    return capture;
};
...
void AudioDestroyCaptureByIdVdi(uint32_t captureId)
{
    ...
    priv->captureInfos[captureId].streamType = 0;
    priv->captureInfos[captureId].sampleRate = 0;
    priv->captureInfos[captureId].channelCount = 0;

    ...
    struct IAudioRender **render, uint32_t *renderId)
{
    ...
    if (desc->pins == PIN_OUT_LINEOUT || desc->pins == PIN_NONE || desc->pins >= PIN_IN_MIC) {
    ...
    uint32_t *renderId, struct IAudioRenderVdi **vdiRender)
{
    ...
    pthread_rwlock_wrlock(GetRenderLock(AUDIO_VDI_STREAM_NUM_MAX));
    ...
    pthread_rwlock_unlock(GetRenderLock(AUDIO_VDI_STREAM_NUM_MAX));
    CancelTimer(id);
    ...
    const struct AudioSampleAttributes *attrs, struct IAudioRender **render, uint32_t *renderId)
{
    ...
    if (vdiAdapter->CheckSupport(desc, attrs) == HDF_ERR_NOT_SUPPORT) {
        AUDIO_FUNC_LOGE("%{public}s not support, portId=%{public}u, pin=%{public}d, desc=%{public}s, type=%{public}d, \
            format=%{public}d, sampleRate=%{public}u, channelCount=%{public}u, steamId=%{public}d, sourceType=%{public}d",
            __func__, desc->portId, desc->pins, desc->desc, attrs->type, attrs->format, attrs->sampleRate,
            attrs->channelCount, attrs->streamId, attrs->sourceType);
...
static int32_t AudioDestroyRenderVdi(struct IAudioAdapter *adapter, uint32_t renderId)
{
    ...
    pthread_rwlock_wrlock(GetRenderLock(renderId));
    if (DecreaseRenderUsrCount(renderId) > 0) {
        pthread_rwlock_unlock(GetRenderLock(renderId));
    ...
    if (vdiRender == NULL || vdiAdapter->DestroyRender == NULL) {
        pthread_rwlock_unlock(GetRenderLock(renderId));
        AUDIO_FUNC_LOGE("invalid parameter renderId:%{public}u", renderId);
        ret = HDF_ERR_INVALID_PARAM;
        goto EXIT;
    }
    ...
    const struct AudioDeviceDescriptor *desc, const struct AudioSampleAttributes *attrs, uint32_t *captureId)
{
    ...
    pthread_rwlock_wrlock(GetCaptureLock(AUDIO_VDI_STREAM_NUM_MAX));
    struct timeval startTime = AudioDfxSysEventGetTimeStamp();
    ...
    pthread_rwlock_unlock(GetCaptureLock(AUDIO_VDI_STREAM_NUM_MAX));
    CancelTimer(id);
    OsalMemFree((void *)vdiDesc.desc);
    if (ret != HDF_SUCCESS) {

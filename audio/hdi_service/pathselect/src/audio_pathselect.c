...
static int32_t AudioCJsonCardServiceItemCheck(
    cJSON *cJsonObj, const char *firstItem, const char *secondItem, const char *thirdItem)
{
...
static int32_t AudioPathSelGetPlanRenderScene(struct AudioHwRenderParam *renderSceneParam)
{
    ...
    ret = AudioCJsonCardServiceItemCheck(
        g_cJsonObj, renderSceneParam->renderMode.hwInfo.cardServiceName, pathName, deviceType);
    if (ret != HDF_SUCCESS) {
        return ret;
    }
    ret = strncpy_s(renderSceneParam->renderMode.hwInfo.pathSelect.useCase, NAME_LEN, useCase, strlen(useCase) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
    }
    ret = strncpy_s(renderSceneParam->renderMode.hwInfo.pathSelect.deviceInfo.deviceType, NAME_LEN, deviceType,
        strlen(deviceType) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
...
static int32_t AudioPathSelGetPlanCaptureScene(struct AudioHwCaptureParam *captureSceneParam)
{
    ...
    ret = AudioCJsonCardServiceItemCheck(
        g_cJsonObj, captureSceneParam->captureMode.hwInfo.cardServiceName, pathName, deviceType);
    if (ret != HDF_SUCCESS) {
        return ret;
    }
    ret = strncpy_s(captureSceneParam->captureMode.hwInfo.pathSelect.useCase, NAME_LEN, useCase, strlen(useCase) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
    }
    ret = strncpy_s(captureSceneParam->captureMode.hwInfo.pathSelect.deviceInfo.deviceType, NAME_LEN, deviceType,
        strlen(deviceType) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
...
static int32_t AudioCJsonParseGetSubItem(
    cJSON *cJsonObj, const char *firstItem, const char *secondItem, cJSON **deviceList)
{
...
static int32_t AudioCapturePathSelGetUsecaseDevice(struct AudioHwCaptureParam *captureParam, const char *pathName)
{
    ...
    while (pathList != NULL) {
        ...
        ret = strncpy_s(captureParam->captureMode.hwInfo.pathSelect.pathPlan[pathIndex].pathPlanName, PATHPLAN_LEN,
            pathKey, strlen(pathKey) + 1);
        if (ret != 0) {
            AUDIO_FUNC_LOGE("strncpy_s failed!");
            return HDF_FAILURE;
        }
...
static int32_t AudioPathSelGetPlanCapture(struct AudioHwCaptureParam *captureParam)
{
    ...
    ret = strncpy_s(captureParam->captureMode.hwInfo.pathSelect.useCase, NAME_LEN, useCase, strlen(useCase) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
    }
    ret = strncpy_s(captureParam->captureMode.hwInfo.pathSelect.deviceInfo.deviceType, NAME_LEN, deviceType,
        strlen(deviceType) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s failed!");
        return HDF_FAILURE;
...
static int32_t AudioRenderPathSelGetUsecaseDevice(struct AudioHwRenderParam *renderParam, const char *pathName)
{
    ...
    while (pathList != NULL) {
        ...
        ret = strncpy_s(renderParam->renderMode.hwInfo.pathSelect.pathPlan[pathIndex].pathPlanName, PATHPLAN_LEN,
            pathKey, strlen(pathKey));
        if (ret != 0) {
...
static int32_t AudioRenderPathSelGetDevice(struct AudioHwRenderParam *renderParam, const char *deviceType)
{
    ...
    ret =
        AudioCJsonParseGetSubItem(g_cJsonObj, renderParam->renderMode.hwInfo.cardServiceName, deviceType, &deviceList);
    if (ret != HDF_SUCCESS) {
        return ret;
    }
    while (deviceList != NULL) {
...
static int32_t AudioPathSelGetPlanRender(struct AudioHwRenderParam *renderParam)
{
    ...
    ret = strncpy_s(renderParam->renderMode.hwInfo.pathSelect.useCase, NAME_LEN, useCase, strlen(useCase) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s is Fail!");
        return HDF_FAILURE;
    }
    ret = strncpy_s(
        renderParam->renderMode.hwInfo.pathSelect.deviceInfo.deviceType, NAME_LEN, deviceType, strlen(deviceType) + 1);
    if (ret != 0) {
        AUDIO_FUNC_LOGE("strncpy_s is Fail!");
        return HDF_FAILURE;
    }

...
void ShowAllAcodecRegister(void)
{
    ...
    AUDIO_DEVICE_LOG_DEBUG("ACODEC REG: AUDIO_CODEC_MASKREG = [%08x]", val);
    // ACODEC REG 0030
    val = AcodecHalReadReg(ACODEC_DIGCTRL1_ADDR);
    AUDIO_DEVICE_LOG_DEBUG("ACODEC REG: ACODEC_DIGCTRL1_ADDR = [%08x]", val);

    val = AcodecHalReadReg(ACODEC_DIGCTRL3_ADDR);
    AUDIO_DEVICE_LOG_DEBUG("ACODEC REG: ACODEC_DIGCTRL3_ADDR = [%08x]", val);
}

...
{
    volatile unsigned int *regAddress = (volatile unsigned int *)addr;
    *regAddress = value;
}
...
int32_t ExternalCodecDeviceInit(struct AudioCard *audioCard, const struct AccessoryDevice *device)
{
    ...
    ret = AudioAddControls(audioCard, g_tfa9879AudioControls, ARRAY_SIZE(g_tfa9879AudioControls));
    if (ret != HDF_SUCCESS) {
...
int32_t ExternalCodecDeviceReadReg(const struct AccessoryDevice *codec, uint32_t reg, uint32_t *val)
{
    int32_t ret;
    struct Tfa9879RegAttr regAttr;
    uint16_t tmpVal;
...
static int32_t FormatToBitWidth(enum AudioFormat format, uint16_t *bitWidth)
{
...
static int32_t RateToFrequency(uint32_t rate, uint16_t *freq)
{
    switch (rate) {

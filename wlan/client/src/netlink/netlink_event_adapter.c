...
{
    HDF_LOGD(LOG_CORE, "hal enter %{public}s", __FUNCTION__);
    struct genlmsghdr *hdr = nlmsg_data(nlmsg_hdr(msg));
    ...
    for (i = 0; i < networkInfo.nums; i++) {
        HDF_LOGD(LOG_CORE, "name=%{public}s index=%{public}d mode=%{public}s",
    ...
    HDF_LOGD(LOG_CORE, "hal exit %{public}s", __FUNCTION__);
    return NL_SKIP;
}

...
namespace Codec {
    bool CodecComponentTunnelRequest(const uint8_t* data, size_t size)
    {
        ...
        if (ret != HDF_SUCCESS) {
            HDF_LOGE("%{public}s: SendCommand Component faild\n", __func__);
            return false;
        }

        ret = component->ComponentTunnelRequest(component, (uint32_t)data, (int32_t)data, (uint32_t)data, (struct OMX_TUNNELSETUPTYPE *)data);
        if (ret == HDF_SUCCESS) {

...
namespace Codec {
    bool CodecGetParameter(const uint8_t *data, size_t size)
    {
        ...
        std::vector<int8_t> inParamStruct, outParamStruct;
        ObjectToVector(params.paramStruct, inParamStruct);

        int32_t ret = g_component->GetParameter(params.paramIndex, inParamStruct, outParamStruct);
        if (ret != HDF_SUCCESS) {

...
bool CodecUseEglImage(const uint8_t *data, size_t size)
{
    ...
    int32_t ret = g_component->UseEglImage(g_component, &buffer, params.portIndex, params.eglImage, params.eglImageLen);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%{public}s: UseEglImage failed", __func__);
    }

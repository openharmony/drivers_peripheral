    ...
    int32_t outputBufferNum, int32_t outputBufferSize)
{
    ...
    for (int32_t i = 0; i < inputBufferNum; i++) {
        ...
        if (CreateFdShareMemory(&g_inputBuffers[i]) != HDF_SUCCESS) {
            return false;
        }
    ...
    for (int32_t j = 0; j < outputBufferNum; j++) {
        ...
        if (CreateGrShareMemory(&bufferHandle, g_cmd, &g_outputBuffers[j]) != HDF_SUCCESS) {
            return false;
        }
...
static int32_t SetDecParameter(void)
{
    ...
    ret = memset_s(&param, sizeof(Param), 0, sizeof(Param));
    if (ret != EOK) {
        HDF_LOGE("%{public}s, memset_s codec type failed!", __func__);
        return HDF_FAILURE;
    }
    ...
    ret = memset_s(&param, sizeof(Param), 0, sizeof(Param));
    if (ret != EOK) {
        HDF_LOGE("%{public}s, memset_s format failed!", __func__);
        return HDF_FAILURE;
    }
    paramCnt = 1;
    ...
    ret = memset_s(&param, sizeof(Param), 0, sizeof(Param));
    if (ret != EOK) {
        HDF_LOGE("%{public}s, memset_s stride failed!", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CodecCallbackInputBufferAvailable(UINTPTR userData, CodecBuffer *inBuf, int32_t *acquireFd)
{
    ...
    if (g_autoSplit == 1) {
        readSize = ReadInputFromFile(g_data.fpInput, sm->virAddr);
        g_totalSrcSize += readSize;
        g_pktEos = (readSize <= 0);
    } else {
        readSize = ReadOneFrameFromFile(g_data.fpInput, sm->virAddr);
        g_totalSrcSize += readSize;
...
static int32_t CodecCallbackOutputBufferAvailable(UINTPTR userData, CodecBuffer *outBuf, int32_t *acquireFd)
{
    ...
    DumpOutputToFile(g_data.fpOutput, sm->virAddr);

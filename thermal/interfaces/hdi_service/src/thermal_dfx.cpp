...
void ThermalDfx::WriteToEmptyFile(std::ofstream& wStream, std::string& currentTime, uint32_t paramWidth)
{
    ...
    for (auto info : logInfo) {
        wStream << info.title;
        if (info.value == logInfo.back().value && info.title == logInfo.back().title) {
            break;
        }
        for (uint32_t i = 0; i < paramWidth - info.title.length(); ++i) {
            wStream << " ";
        }
    }
    wStream << "\n";
...
void ThermalDfx::WriteToFile(std::ofstream& wStream, std::string& currentTime, uint32_t paramWidth)
{
    ...
    for (auto info : logInfo) {
        ...
        ThermalHdfUtils::GetInstance().ReadNode(valuePath, value);
        wStream << value;
        if (info.value == logInfo.back().value && info.title == logInfo.back().title) {
            break;
        }
        for (uint32_t i = 0; i < paramWidth - info.value.length(); ++i) {
            wStream << " ";
        }
    }
    wStream << "\n";

...
int32_t OpenEp0AndMapAddr(struct UsbFnFuncMgr *funcMgr)
{
    ...
    if (funcMgr->fd <= 0) {
        HDF_LOGE("%{public}s:%d openPipe failed", __func__, __LINE__);
        return HDF_ERR_IO;
    }

    ret = fnOps->queueInit(funcMgr->fd);
    if (ret) {
        HDF_LOGE("%{public}s:%d queueInit failed", __func__, __LINE__);
        return HDF_ERR_IO;
    }
    return 0;
}
...
int32_t UsbFnIoMgrRequestFree(struct UsbFnRequest *req)
{
    ...
    if (ret) {
        HDF_LOGE("%{public}s:%d ummapAddr failed, ret=%d ", __func__, __LINE__, ret);
...
static int32_t HandleInit(struct UsbHandleMgr *handle, struct UsbFnInterfaceMgr *interfaceMgr)
{
    ...
    for (i = 0; i < handle->numFd; i++) {
        ...
        if (ret) {
            HDF_LOGE("%{public}s: queueInit failed ret = %d", __func__, ret);
            return HDF_ERR_IO;
        }

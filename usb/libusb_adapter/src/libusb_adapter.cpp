...
int32_t LibusbAdapter::CloseDevice(const UsbDev &dev, bool isDetach)
{
    HDF_LOGI("%{public}s enter", __func__);
    libusb_device_handle *devHandle = nullptr;
    ...
    std::vector<uint8_t> &data)
{
    ...
    int actlength = 0;
    libusb_device_handle *devHandle = nullptr;
    StartTraceEx(HITRACE_LEVEL_INFO, HITRACE_TAG_USB, "FindHandleByDev");
    int32_t ret = FindHandleByDev(dev, &devHandle);
    ...
    const UsbDev &dev, const UsbPipe &pipe, int32_t timeout, std::vector<uint8_t> &data)
{
    ...
    StartTraceEx(HITRACE_LEVEL_INFO, HITRACE_TAG_USB, "libusb_interrupt_transfer");
...
int32_t LibusbAdapter::GetConfigDescriptor(libusb_device *dev, uint8_t descId, std::vector<uint8_t> &descriptor)
{
    ...
    int32_t ret = memcpy_s(descriptor.data() + currentOffset, configLen, config, configLen);
    if (ret != EOK) {
        HDF_LOGE("%{public}s: memcpy_s failed", __func__);
    ...
    size_t &currentOffset)
{
    ...
    for (int32_t i = 0; i < iface->num_altsetting; ++i) {
        ...
        int32_t ret = memcpy_s(descriptor.data() + currentOffset, ifaceLen, &altSetting, ifaceLen);
        if (ret != EOK) {
            HDF_LOGE("%{public}s: memcpy_s failed", __func__);
            return HDF_FAILURE;
        }
        ...
        for (int32_t j = 0; j < altSetting.bNumEndpoints; ++j) {
            ...
            ret = memcpy_s(descriptor.data() + currentOffset, epLen, &endpoint, epLen);
            if (ret != EOK) {
                HDF_LOGE("%{public}s: memcpy_s failed", __func__);
                return HDF_FAILURE;
            }
...
int32_t LibusbAdapter::GetDeviceMemMapFd(const UsbDev &dev, int &fd)
{
    ...
    fdsan_exchange_owner_tag(ret, 0, fdsan_create_owner_tag(FDSAN_OWNER_TYPE_FILE, LOG_DOMAIN));
    fd = ret;
    {
        std::unique_lock<std::shared_mutex> lock(g_mapMutexUsbOpenFdMap);
        g_usbOpenFdMap[result] = fd;
...
void HotplugEventPorcess::OnProcessHotplugEvent()
{
    while (!shutdown_) {
        ...
        if (info.hotplugInfo.status == ACT_DEVDOWN) {
            UsbDev dev = {info.hotplugInfo.busNum, info.hotplugInfo.devNum};
            LibusbAdapter::GetInstance()->CloseDevice(dev, true);
        }

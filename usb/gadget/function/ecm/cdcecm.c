...
static void UsbEcmRxPush(struct UsbEcm *port)
{
    ...
    while (!DListIsEmpty(queue)) {
        ...
        if (req->actual && req->status == 0) {
            ...
            if (count != size) {
                HDF_LOGW("%{public}s: write %{public}d less than expected %{public}d",
                    __func__, count, size);
            }
...
static void UsbEcmFreeRequests(struct DListHead *head, int *allocated)
{
    struct UsbFnRequest *req = NULL;
...
static int32_t UsbEcmAllocReadRequests(struct UsbEcm *port, int num)
{
...
static int32_t UsbEcmAllocWriteRequests(struct UsbEcm *port, int num)
{
    struct UsbEcmDevice *ecm = port->ecm;
...
static void EcmDoNotify(struct UsbEcmDevice *ecm)
{
    ...
    switch (ecm->notifyState) {
        ...
        case ECM_NOTIFY_CONNECT:
            ...
            HDF_LOGD("notify connect %{public}s\n", ecm->isOpen ? "true" : "false");
        ...
        case ECM_NOTIFY_SPEED:
            ...
            HDF_LOGD("notify speed %{public}d\n", EcmBitrate());
...
static void EcmNotifyComplete(uint8_t pipe, struct UsbFnRequest *req)
{
    ...
    switch (req->status) {
        case 0:
            ...
            break;
        default:
            HDF_LOGD("event %{public}d --> %{public}d\n",
                event->bNotificationType, req->status);
            break;
    }
}

static int EcmSetup(struct UsbEcmDevice *ecm, struct UsbFnCtrlRequest *ctrl)
{
    ...
    switch ((ctrl->reqType << 0x08) | ctrl->request) {
        case ((USB_DDK_DIR_OUT | USB_DDK_TYPE_CLASS | USB_DDK_RECIP_INTERFACE) << 0x08)
            ...
            if (length != 0 || index != ecm->ctrlId)
                goto invalid;
            HDF_LOGD("packet filter %{public}02x\n", value);
            ret = 0;
            break;

        default:
invalid:
        HDF_LOGD("invalid control req%{public}02x.%{public}02x v%{public}04x i%{public}04x l%{public}d\n",
    ...
    if (ret >= 0) {
        ...
        if (ret < 0) {
            HDF_LOGD("ecm req %{public}02x.%{public}02x response err %{public}d\n",
...
static int EcmEnable(struct UsbEcmDevice *ecm)
{
    ...
    if (port->refCount) {
        ret = UsbEcmStartIo(port);
        if (ret != HDF_SUCCESS) {
            HDF_LOGE("%{public}s: UsbEcmStartIo failed", __func__);
        }
    }
    EcmNotify(ecm);
    OsalMutexUnlock(&port->lock);
...
static void EcmDisable(struct UsbEcmDevice *ecm)
{
    struct UsbEcm *port = ecm->port;

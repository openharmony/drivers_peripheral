...
#include "usbgetfiledescriptor_fuzzer.h"
...
namespace USB {
    ...
    bool UsbGetFileDescriptorFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().GetFileDescriptor(dev, reinterpret_cast<int32_t &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: get file descriptor succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbGetFileDescriptorFuzzTest(data, size);
    return 0;
}
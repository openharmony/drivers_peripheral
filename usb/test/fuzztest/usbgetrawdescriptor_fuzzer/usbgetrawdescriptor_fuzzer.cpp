...
#include "usbgetrawdescriptor_fuzzer.h"
...
namespace USB {
    ...
    bool UsbGetRawDescriptorFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().GetRawDescriptor(dev, reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: get raw descriptor succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbGetRawDescriptorFuzzTest(data, size);
    return 0;
}
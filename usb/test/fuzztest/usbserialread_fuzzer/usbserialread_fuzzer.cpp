...
#include "usbserialread_fuzzer.h"
...
bool UsbSerialReadFuzzTest(const uint8_t *data, size_t size)
{
    (void) data;
    (void) size;
    ...
    ret = usbSerialInterface->Read(*device, buffersize, buff);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::USBSerial::UsbSerialReadFuzzTest(data, size);
    return 0;
}
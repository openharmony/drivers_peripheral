...
bool ScsiRead10FuzzTest(const uint8_t *data, size_t size)
{
    ...
    std::vector<uint8_t> capacityInfo;
    ScsiPeripheralResponse response;
    ret = scsiPeripheralDdk->Read10(device, request, response);
...
{
    OHOS::SCSI::ScsiRead10FuzzTest(data, size);
    return 0;
}
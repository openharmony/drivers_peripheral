...
#include "scsirequestsense_fuzzer.h"
#include <cstring>
#include <securec.h>
#include <scsi/sg.h>
#include "hdf_log.h"
...
namespace OHOS {
namespace SCSI {
static void ConstructParamsFromData(ScsiPeripheralRequestSenseRequest& request, const uint8_t* data, size_t size)
{
    if (data == nullptr || size < sizeof(uint8_t)) {
        return;
    }

    request.allocationLength = *data;
...
bool ScsiRequestSenseFuzzTest(const uint8_t *data, size_t size)
{
    ...
    ScsiPeripheralRequestSenseRequest request = {};
    ConstructParamsFromData(request, data, size);
    ScsiPeripheralResponse response;
    ret = scsiPeripheralDdk->RequestSense(device, request, response);
...
{
    OHOS::SCSI::ScsiRequestSenseFuzzTest(data, size);
    return 0;
}
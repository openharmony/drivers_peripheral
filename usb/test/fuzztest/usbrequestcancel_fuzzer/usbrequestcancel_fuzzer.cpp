...
#include "usbrequestcancel_fuzzer.h"
...
namespace USB {
    ...
    bool UsbRequestCancelFuzzTest(const uint8_t* data, size_t size)
    {
        (void)size;
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().RequestCancel(dev, reinterpret_cast<const UsbPipe &>(data));
        if (ret == UEC_OK) {
...
{
    OHOS::USB::UsbRequestCancelFuzzTest(data, size);
    return 0;
}
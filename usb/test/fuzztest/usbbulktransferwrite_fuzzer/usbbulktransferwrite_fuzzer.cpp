...
bool UsbBulkTransferWriteFuzzTest(const uint8_t *data, size_t size)
{
    ...
    ret = usbInterface->BulkTransferWrite(dev, pipe, timeout, reinterpret_cast<const std::vector<uint8_t> &>(data));
    if (ret == HDF_SUCCESS) {
        HDF_LOGI("%{public}s: bulk transfer Write succeed", __func__);
    }

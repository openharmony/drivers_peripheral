...
#include "scsireadcapacity10_fuzzer.h"
#include <cstring>
#include <securec.h>
#include <scsi/sg.h>
#include <unistd.h>
...
namespace OHOS {
namespace SCSI {
static void ConstructParamsFromData(ScsiPeripheralReadCapacityRequest& request, const uint8_t* data, size_t size)
{
    if (data == nullptr || size < sizeof(uint32_t)) {
        return;
    }

    (void)memcpy_s(&request.lbAddress, sizeof(uint32_t), data, sizeof(uint32_t));
...
bool ScsiReadCapacity10FuzzTest(const uint8_t *data, size_t size)
{
    ...
    ret = scsiPeripheralDdk->ReadCapacity10(device, request, capacityInfo, response);
...
{
    OHOS::SCSI::ScsiReadCapacity10FuzzTest(data, size);
    return 0;
}
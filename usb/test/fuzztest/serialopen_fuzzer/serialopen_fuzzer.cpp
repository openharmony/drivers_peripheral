...
namespace OHOS {
namespace SERIAL {
    bool SerialOpenFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        if (serialInterface->SerialOpen(portId) != OK) {
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SERIAL::SerialOpenFuzzTest(data, size);
    return 0;
}

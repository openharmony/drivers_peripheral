...
#include "usbmanageinterface_fuzzer.h"
...
bool UsbManageInterfaceFuzzTest(const uint8_t *data, size_t size)
{
    (void)size;
    UsbDev dev;
    ...
    ret = usbInterface->ManageInterface(dev, param.interfaceId, param.disable);
    if (ret == HDF_SUCCESS) {
        HDF_LOGI("%{public}s: manage interface succeed", __func__);
    }
...
{
    OHOS::USB::UsbManageInterfaceFuzzTest(data, size);
    return 0;
}
...
#include "usbinterrupttransferread_fuzzer.h"
...
namespace OHOS {
namespace USB {
bool UsbInterruptTransferReadFuzzTest(const uint8_t *data, size_t size)
{
    (void)size;
    UsbDev dev;
    ...
    ret = usbInterface->InterruptTransferRead(dev, pipe, timeout, reinterpret_cast<std::vector<uint8_t> &>(data));
    if (ret == HDF_SUCCESS) {
        HDF_LOGI("%{public}s: interrupt transfer read succeed", __func__);
    }
...
{
    OHOS::USB::UsbInterruptTransferReadFuzzTest(data, size);
    return 0;
}
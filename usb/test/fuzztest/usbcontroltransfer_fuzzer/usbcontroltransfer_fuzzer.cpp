...
#include "usbcontroltransfer_fuzzer.h"
...
namespace USB {
    ...
    bool UsbControlTransferFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().ControlTransfer(dev, reinterpret_cast<const UsbCtrlTransfer &>(data),
            reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: control transfer succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbControlTransferFuzzTest(data, size);
    return 0;
}
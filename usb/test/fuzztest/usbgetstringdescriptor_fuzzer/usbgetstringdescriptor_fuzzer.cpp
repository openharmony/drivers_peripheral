...
#include "usbgetstringdescriptor_fuzzer.h"
...
namespace USB {
    ...
    bool UsbGetStringDescriptorFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().GetStringDescriptor(dev, descId,
            reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: get string descriptor succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbGetStringDescriptorFuzzTest(data, size);
    return 0;
}
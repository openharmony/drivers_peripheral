...
namespace USB {
    ...
    bool UsbGetConfigFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: get config succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbGetConfigFuzzTest(data, size);
    return 0;
}
...
#include "usbisotransferwrite_fuzzer.h"
...
namespace USB {
bool UsbIsoTransferWriteFuzzTest(const uint8_t *data, size_t size)
{
    (void)size;
    UsbDev dev;
    ...
    ret = usbInterface->IsoTransferWrite(
        dev, pipe, timeout, reinterpret_cast<std::vector<uint8_t> &>(std::move(data + OFFSET_BYTE)));
    if (ret == HDF_SUCCESS) {
        HDF_LOGI("%{public}s: iso transfer write succeed", __func__);
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::USB::UsbIsoTransferWriteFuzzTest(data, size);
    return 0;
}
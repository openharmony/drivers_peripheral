...
#include "usbbulktransferread_fuzzer.h"
...
namespace USB {
    ...
    bool UsbBulkTransferReadFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        ret = UsbdClient::GetInstance().BulkTransferRead(dev, reinterpret_cast<const UsbPipe &>(data),
            *(int32_t *)data, reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: bulk transfer read succeed\n", __func__);
            result = true;
        }

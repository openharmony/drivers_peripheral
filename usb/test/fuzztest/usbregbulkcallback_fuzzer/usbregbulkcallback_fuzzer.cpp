...
#include "usbregbulkcallback_fuzzer.h"
...
namespace USB {
    ...
    bool UsbRegBulkCallbackFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        ret = UsbdClient::GetInstance().RegBulkCallback(dev,  reinterpret_cast<const UsbPipe &>(data),
            reinterpret_cast<const sptr<IRemoteObject> &>(data));
        if (ret == UEC_OK) {

...
#include "usbisotransferread_fuzzer.h"
...
namespace USB {
    ...
    bool UsbIsoTransferReadFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().IsoTransferRead(dev, reinterpret_cast<const UsbPipe &>(data),
            *(int32_t *)data, reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: iso transfer read succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbIsoTransferReadFuzzTest(data, size);
    return 0;
}
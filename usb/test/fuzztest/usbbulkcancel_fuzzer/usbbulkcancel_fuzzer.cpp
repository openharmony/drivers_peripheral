...
#include "usbbulkcancel_fuzzer.h"
...
namespace USB {
    ...
    bool UsbBulkCancelFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().BulkCancel(dev, reinterpret_cast<const UsbPipe &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: bulk cancel succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbBulkCancelFuzzTest(data, size);
    return 0;
}
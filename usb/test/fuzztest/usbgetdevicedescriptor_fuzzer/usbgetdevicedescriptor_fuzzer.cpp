...
#include "usbgetdevicedescriptor_fuzzer.h"
...
namespace USB {
    ...
    bool UsbGetDeviceDescriptorFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        ...
        ret = UsbdClient::GetInstance().GetDeviceDescriptor(dev, reinterpret_cast<std::vector<uint8_t> &>(data));
        if (ret == UEC_OK) {
            HDF_LOGI("%{public}s: get device descriptor succeed\n", __func__);
            result = true;
        }
...
{
    OHOS::USB::UsbGetDeviceDescriptorFuzzTest(data, size);
    return 0;
}
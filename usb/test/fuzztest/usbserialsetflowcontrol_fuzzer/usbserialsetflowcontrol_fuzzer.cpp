...
#include "usbserialsetflowcontrol_fuzzer.h"
...
static uint32_t GetFlowControl(const uint8_t* data, size_t size)
{
...
bool UsbSerialSetBaudRateFuzzTest(const uint8_t *data, size_t size)
{
    ...
    enum UsbSerialFlowControl flowControl = static_cast<UsbSerialFlowControl>(GetFlowControl(data, size));
    ret = usbSerialInterface->SetFlowControl(*device, flowControl);
    if (ret != HDF_SUCCESS) {

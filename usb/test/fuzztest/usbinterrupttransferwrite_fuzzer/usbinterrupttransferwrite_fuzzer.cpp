...
#include "usbinterrupttransferwrite_fuzzer.h"
...
namespace USB {
bool UsbInterruptTransferWriteFuzzTest(const uint8_t *data, size_t size)
{
    (void)size;
    UsbDev dev;
    ...
    ret = usbInterface->InterruptTransferWrite(
        dev, pipe, timeout, reinterpret_cast<std::vector<uint8_t> &>(std::move(data + OFFSET_BYTE)));
    if (ret == HDF_SUCCESS) {
        HDF_LOGI("%{public}s: interrupt transfer write succeed", __func__);
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::USB::UsbInterruptTransferWriteFuzzTest(data, size);
    return 0;
}
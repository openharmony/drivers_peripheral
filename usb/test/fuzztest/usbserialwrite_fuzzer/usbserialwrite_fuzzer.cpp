...
#include "usbserialwrite_fuzzer.h"
...
bool UsbSerialWriteFuzzTest(const uint8_t *data, size_t size)
{
    ...
    buff.assign(data, data + size); // copy data to buff
    ret = usbSerialInterface->Write(*device, buff, bytesWritten);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::USBSerial::UsbSerialWriteFuzzTest(data, size);
    return 0;
}
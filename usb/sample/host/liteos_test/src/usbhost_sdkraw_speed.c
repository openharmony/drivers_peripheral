...
static int32_t UsbIoThread(void *data)
{
    ...
    while (true) {
        ...
        if (ret < 0) {
            printf("%s:%d UsbRawHandleRequests failed, ret=%d ", __func__, __LINE__, ret);
...
static int32_t UsbStopIo(struct AcmDevice * const acm)
{
    ...
    if (ret != HDF_SUCCESS) {
        printf("%s:%d OsalThreadDestroy failed, ret=%d", __func__, __LINE__, ret);
        return ret;
    }
...
static int32_t UsbGetConfigDescriptor(UsbRawHandle *devHandle, struct UsbRawConfigDescriptor ** const config)
{
...
static int32_t UsbSpeedGetBulkEndpoint(struct AcmDevice * const acm, const struct UsbRawEndpointDescriptor *endPoint)
{
...
static void AcmTestBulkCallback(const void *requestArg)
{
    ...
    if (g_printData) {
        ...
        fflush(stdout);
    } else if (g_recv_count % USB_RECV_COUNT_TSH == 0) {
        printf("#");
        fflush(stdout);
    }
...
static int32_t UsbSerialSpeedInit(const struct UsbSpeedTest *input, int32_t * const ifaceNum)
{
...
static void UsbSpeedDdkExit(const struct UsbSession *session)
{
    ...
    if (UsbStopIo(g_acm) != HDF_SUCCESS) {
        HDF_LOGE("%{public}s UsbStopIo error!", __func__);
    }
    if (UsbFreeDataRequests(g_acm) != HDF_SUCCESS) {
        HDF_LOGE("%{public}s UsbFreeDataRequests error!", __func__);
    }
    AcmDataBufFree(g_acm);

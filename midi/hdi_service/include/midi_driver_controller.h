...
struct OutputContext {
    ...
    uint32_t portIndex;
    bool isUmp;
    // 使用联合体或根据 isUmp 区分使用哪个指针
    snd_rawmidi_t *rawmidi = nullptr;
    snd_ump_t *ump = nullptr;
};
...
private:
    ...
    void EnumerationDeviceMidi2();
    void CleanupDeviceInputPorts(int64_t deviceId);
    void CleanupRemovedDevices(const std::vector<DeviceInfo> &oldDeviceList);
    std::shared_ptr<InputThreadContext> FindInputContext(int64_t deviceId, uint32_t portId);
    std::shared_ptr<OutputContext> FindOutputContext(int64_t deviceId, uint32_t portId);
    void InputThread(std::shared_ptr<InputThreadContext> ctx);
    std::vector<DeviceInfo> deviceList_;
    std::vector<int64_t> activeDevice_;
    std::vector<std::shared_ptr<InputThreadContext>> inputCtxs;
    std::vector<std::shared_ptr<OutputContext>> outputCtxs_;
